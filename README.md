...
- **Two models, two datasets**: enough to show the TruthfulQA result isn't one model's quirk and isn't dataset-independent, not enough to claim broad generalization across model scales, families, or task types. `configs/models.yaml`/`configs/datasets.yaml` make adding more of each straightforward if that evidence is needed later.
- **Single seed**: every run uses `CFG.seed=42` throughout, for exact reproducibility rather than variance estimation across seeds. The bootstrap subset-size sensitivity check is the only source of variance information currently in the project.
